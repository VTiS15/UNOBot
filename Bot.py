...
async def initialize():
    ...
    if not os.path.exists('dgs.json'):
        with open('dgs.json', 'w') as t:
            t.write('{}')
    if not os.path.exists('users.json'):
        with open('users.json', 'w') as t:
            t.write('{}')
    if not os.path.exists('commands.json'):
        with open('commands.json', 'w') as t:
            t.write('{}')

    if client.guilds:
        with open('dgs.json') as dgs_json:
            dgs = json.load(dgs_json)

        if not dgs:
            for guild in client.guilds:
                dgs[str(guild.id)] = default_dgs
            with open('dgs.json', 'w') as dgs_json:
                json.dump(dgs, dgs_json)

        else:
            new_dgs = {}
            for guild in client.guilds:
                if str(guild.id) not in dgs:
                    new_dgs[str(guild.id)] = default_dgs
            with open('dgs.json', 'w') as dgs_json:
                dgs.update(new_dgs)
                json.dump(dgs, dgs_json)
                dgs_json.truncate()

        with open('users.json') as options_json:
            user_stuff = json.load(options_json)

        if not user_stuff:
            for member in client.get_all_members():
                if member.id != client.user.id and not member.bot:
            ...
            with open('users.json', 'w') as options_json:
                json.dump(user_stuff, options_json)

        else:
            new_options = {}
            for member in client.get_all_members():
                if str(member.id) not in user_stuff and member.id != client.user.id and not member.bot:
                    default_user_stuff = { "AllowAlerts": True }
                    for guild in client.guilds:
                        if guild.get_member(member.id):
                            default_user_stuff[str(guild.id)] = {
                    ...
                    new_options[str(member.id)] = default_user_stuff

            with open('users.json', 'w') as options_json:
                user_stuff.update(new_options)
                json.dump(user_stuff, options_json)
                options_json.truncate()
        ...
        if not commands:
            ...
            with open('commands.json', 'w') as commands_json:
                json.dump(commands, commands_json)

        else:
            new_command_settings = {}
            for guild in client.guilds:
                if str(guild.id) not in commands:
                    new_command_settings[str(guild.id)] = default_command_settings
            with open('commands.json', 'w') as commands_json:
                commands.update(new_command_settings)
                json.dump(commands, commands_json)
                commands_json.truncate()

        for guild in client.guilds:
            cooldowns[str(guild.id)] = []

    else:
        with open('dgs.json', 'w') as dgs:
            json.dump({}, dgs)
            dgs.truncate()

        with open('users.json', 'w') as options_json:
            json.dump({}, options_json)
            options_json.truncate()

        with open('commands.json', 'w') as commands_json:
            json.dump({}, commands_json)
            commands_json.truncate()
...
def has_played(user):
    ...
    for guild in [x for x in client.guilds if x.get_member(user.id)]:
        with open('users.json') as users_json:
            if json.load(users_json)[str(user.id)][str(guild.id)]['Played'] > 0:
                return True
...
async def game_shutdown(d, winner: discord.Member=None, guild=None):
    ...
    if winner and not guild:
        ...
        with open('users.json', 'r+') as users_json:
            ...
            for id in player_ids:
                users[id][str(guild.id)]['Played'] += 1
        ...
        if not DUM:
            ...
            if not games[str(guild.id)]['settings']['Flip']:
                ...
                if not games[str(guild.id)]['dark']:
                    ...
                    while color != current_color and value != current_value or not any(x in c[0] for x in ('+2', 'wild')):
                        c = choice(games[str(guild.id)]['cards'])
                    ...
                    while color != current_color and value != current_value or not any(x in c[1] for x in ('+color', 'wild')):
                        c = choice(games[str(guild.id)]['cards'])
...
async def on_guild_join(guild):
    await initialize()
    print('[' + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' | UNOBot] UNOBot has joined the guild ' + str(guild) + '.')
...
async def on_guild_remove(guild):
    await initialize()
    print('[' + datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' | UNOBot] UNOBot has left the guild ' + str(guild) + '.')
...
async def on_member_join(member):
    ...
    if not member.bot:
        with open('users.json', 'r+') as options_json:
            user_stuff = json.load(options_json)
            ...
            if str(member.id) in user_stuff:
                ...
                new_options = {member.id: default_user_stuff}
...
async def on_member_remove(member):
    ...
    if not member.bot:
        with open('users.json') as options_json:
            user_options = json.load(options_json)

        for guild in [x for x in client.guilds if
                      not x.get_member(member.id) and str(x.id) in user_options[str(member.id)]]:
            del user_options[str(member.id)][str(guild.id)]

            if no_guild(member):
                del user_options[str(member.id)]

        with open('users.json', 'w') as options_json:
            json.dump(user_options, options_json)
            options_json.truncate()
                ...
                if int(games[str(message.guild.id)]['player']) == message.author.id and str(
                    ...
                    try:
                        if value in [str(x) for x in range(10)]:
                            ...
                            if not games[str(message.guild.id)]['settings']['Flip']:
                                if color + 'reverse' in games[str(message.guild.id)][str(message.author.id)]['cards']:
                                    if (current_color == color or current_value == 'reverse') and not (
                                        ...
                                        await message.channel.send(
                                            embed=discord.Embed(description=':x: **You can\'t play a ' + color.capitalize() + 'Reverse here!**',
                                                                color=discord.Color.red()))
                                ...
                                if not games[str(message.guild.id)]['dark']:
                                    if color + 'reverse' in [x[0] for x in games[str(message.guild.id)][str(message.author.id)]['cards']]:
                                        if (current_color == color or current_value == 'reverse') and not (
                                                '+' in current_value and games[str(message.guild.id)]['settings'][
                                            'StackCards'] and str(message.guild.id) in stack):
                                            await play_card(choice([x for x in games[str(message.guild.id)][
                                                str(message.author.id)]['cards'] if x[0] == color + 'reverse']), message.author)
                                    ...
                                    if color + 'reverse' in [x[1] for x in games[str(message.guild.id)][str(message.author.id)]['cards']]:
                                        if (current_color == color or current_value == 'reverse') and not (
                                                '+' in current_value and games[str(message.guild.id)]['settings'][
                                            'StackCards'] and str(message.guild.id) in stack):
                                            await play_card(choice([x for x in games[str(message.guild.id)][
                                                str(message.author.id)]['cards'] if x[1] == color + 'reverse']), message.author)
                            ...
                            if not games[str(message.guild.id)]['settings']['Flip']:
                                if color + 'skip' in games[str(message.guild.id)][str(message.author.id)]['cards']:
                                    if (current_color == color or current_value == 'skip') and not (
                                        ...
                                        await message.channel.send(
                                            embed=discord.Embed(description=':x: **You cannot play a ' + color.capitalize() + 'Skip here!**',
                                                                color=discord.Color.red()))
                                ...
                                if not games[str(message.guild.id)]['dark']:
                                    if color + 'skip' in [x[0] for x in games[str(message.guild.id)][str(message.author.id)]['cards']]:
                                        if (current_color == color or current_value == 'skip') and not (
                                                '+' in current_value and games[str(message.guild.id)]['settings'][
                                            'StackCards'] and str(message.guild.id) in stack):
                                            await play_card(choice([x for x in games[str(message.guild.id)][
                                                str(message.author.id)]['cards'] if x[0] == color + 'skip']), message.author)
                                    ...
                                    if color + 'skip' in [x[1] for x in games[str(message.guild.id)][str(message.author.id)]['cards']]:
                                        if (current_color == color or current_value == 'skip') and not (
                                                '+' in current_value and games[str(message.guild.id)]['settings'][
                                            'StackCards'] and str(message.guild.id) in stack):
                                            await play_card(choice([x for x in games[str(message.guild.id)][
                                                str(message.author.id)]['cards'] if x[1] == color + 'skip']), message.author)
                                ...
                                if not games[str(message.guild.id)]['dark']:
                                    if any(x[0] == 'wild' for x in
                                        ...
                                        if color in ('red', 'blue', 'green', 'yellow'):
                                            if not ('+' in current_value and games[str(message.guild.id)]['settings'][
                                                'StackCards'] and str(message.guild.id) in stack):
                                                await play_card((color + 'wild', choice([x for x in games[str(message.guild.id)][
                                                        str(message.author.id)]['cards'] if x[0] == 'wild'])[1]), message.author)
                                                await display_cards(n)
                            ...
                            if not games[str(message.guild.id)]['settings']['Flip']:
                                if color + '+2' in games[str(message.guild.id)][str(message.author.id)]['cards']:
                                    if (current_color == color or current_value == '+2') and not (
                                        ...
                                        await message.channel.send(
                                            embed=discord.Embed(description=':x: **You can\'t play a ' + color.capitalize() + '+2 here!**',
                                                                color=discord.Color.red()))
                                ...
                                if not games[str(message.guild.id)]['dark']:
                                    if any(x[0] == '+2' for x in games[str(message.guild.id)][str(message.author.id)]['cards']):
                                        if color in ('red', 'blue', 'green', 'yellow'):
                                            await play_card((color + '+2', choice([x for x in games[str(message.guild.id)][
                            ...
                            if games[str(message.guild.id)]['settings']['Flip'] and not games[str(message.guild.id)]['dark']:
                                if any(color + '+1' in card[0] for card in games[str(message.guild.id)][str(message.author.id)]['cards']):
                                    if (current_color == color or current_value == '+1') and not (
                                        ...
                                        if games[str(message.guild.id)]['settings']['StackCards'] and (
                                                any('+1' in card[0] for card in
                                                    games[str(message.guild.id)][str(n.id)]['cards']) or any(
                                            card[0] == '+2' for card in games[str(message.guild.id)][str(n.id)]['cards'])):
                                            for channel in message.channel.category.text_channels:
                                                await channel.send(embed=discord.Embed(
                                                    description='**' + n.name + ' can choose to stack cards or draw ' + str(
                                                        stack[str(message.guild.id)]) + ' cards.**',
                            ...
                            if games[str(message.guild.id)]['settings']['Flip'] and games[str(message.guild.id)][
                                'dark']:
                                if any(color + '+5' in card[1] for card in games[str(message.guild.id)][str(message.author.id)]['cards']):
                                    if current_color == color or current_value == '+5':
                                        ...
                                        if games[str(message.guild.id)]['settings']['StackCards'] and any('+5' in card[1] for card in games[str(message.guild.id)][str(n.id)]['cards']):
                                            for channel in message.channel.category.text_channels:
                                                await channel.send(embed=discord.Embed(
                                                    description='**' + n.name + ' can choose to stack cards or draw ' + str(
                                                        stack[str(message.guild.id)]) + ' cards.**',
                            ...
                            if games[str(message.guild.id)]['settings']['Flip'] and games[str(message.guild.id)][
                                'dark'] or not (
                                    '+' in current_value and games[str(message.guild.id)]['settings'][
                                'StackCards'] and str(message.guild.id) in stack):
                                if any('+color' in x[1] for x in games[str(message.guild.id)][str(message.author.id)]['cards']):
                                    await play_card((choice([x for x in games[str(message.guild.id)][
                                    ...
                                    if color + 'flip' in [x[0] for x in
                                        ...
                                        if color == current_color or current_value == 'flip':
                                            ...
                                            games[str(message.guild.id)]['dark'] = not games[str(message.guild.id)][
                                                'dark']
                                            games[str(message.guild.id)]['current'] = games[str(message.guild.id)]['current_opposite']
...
async def on_raw_reaction_add(payload):
    ...
    if message.author == client.user and not payload.member.bot and (
        ...
        if str(payload.emoji) == '✋':
            with open('users.json') as users_json:
                user_options = json.load(users_json)
        ...
        if commands[str(ctx.guild.id)]['stats']['Enabled']:
            ...
            commands[str(ctx.guild.id)]['stats']['Blacklist'])
                ...
                if users[str(user.id)][str(ctx.guild.id)]['Played'] > 0:
                    message = discord.Embed(title=user.name + '\'s Stats in ' + ctx.guild.name, color=discord.Color.red())
                    ...
                    message.add_field(name='Win Percentage',
                                      value='Won **' + str(round(dict['Wins'] / dict['Played'] * 100)) + '%** of games.',
                                      inline=False)
                ...
                if commands[str(ctx.guild.id)]['stats']['Cooldown'] > 0:
                    ...
                    def check(message):
                        return len(message.content.split()) == 6 \
                               ...
                               prefix + 'settings', prefix + 'set', prefix + 'sett', prefix + 'stngs', prefix + 'setting',
                               prefix + 'stng') \
            ...
            commands[str(ctx.guild.id)]['globalstats']['Blacklist'])
                or ctx.message.author.id not in commands[str(ctx.guild.id)]['globalstats']['Blacklist']) \
                    and (not commands[str(ctx.guild.id)]['globalstats']['WhitelistEnabled'] or
                         ...
                         commands[str(ctx.guild.id)]['globalstats']['Whitelist']) or ctx.message.author == ctx.guild.owner:
            ...
            commands[str(ctx.guild.id)]['leaderboard']['Blacklist'])
                or ctx.message.author.id not in commands[str(ctx.guild.id)]['leaderboard']['Blacklist']) \
                    and (not commands[str(ctx.guild.id)]['leaderboard']['WhitelistEnabled'] or
                         ...
                         commands[str(ctx.guild.id)]['leaderboard']['Whitelist']) or ctx.message.author == ctx.guild.owner:
                ...
                for i in range(1, len(leaderboard) + 1):
                    for index in list_duplicates_of(leaderboard, i):
                        user = client.get_user(int([x for x in list(users.keys()) if ctx.guild.get_member(int(x))][index]))
            ...
            commands[str(ctx.guild.id)]['allowalerts']['Blacklist'])
                or ctx.message.author.id not in commands[str(ctx.guild.id)]['allowalerts']['Blacklist']) \
                    and (not commands[str(ctx.guild.id)]['allowalerts']['WhitelistEnabled'] or
                         ...
                         commands[str(ctx.guild.id)]['allowalerts']['Whitelist']) or ctx.message.author == ctx.guild.owner:
                if not option:
                    message = discord.Embed(title=prefix + 'allowalerts', color=discord.Color.red())
                    message.add_field(name='Description:', value='Allows alerts just for you', inline=False)
                    message.add_field(name='Usage:', value='`' + prefix + 'allowalerts <on|off|view>`', inline=False)
                                ...
                                try:
                                    ...
                                    if a[i] in (
                                        ...
                                        if a[i - 1] == 'StartingCards':
                                            try:
                                                if 3 < int(a[i + 1]) < 15:
                                                    games[str(ctx.guild.id)]['settings']['StartingCards'] = int(a[i + 1])
            ...
            commands[str(ctx.guild.id)]['leavegame']['Blacklist'])
                or ctx.message.author.id not in commands[str(ctx.guild.id)]['leavegame']['Blacklist']) \
                    and (not commands[str(ctx.guild.id)]['leavegame']['WhitelistEnabled'] or
                         ...
                         commands[str(ctx.guild.id)]['leavegame']['Whitelist']) or ctx.message.author == ctx.guild.owner:
                if ctx.channel.category.name == 'UNO-GAME':
